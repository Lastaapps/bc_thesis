...
%
\todo[inline]{Ukázat, že \( Y_t \) je prázdný pro všechny, viz \cite{book_parametrized_algorithms}}
...
\begin{proof}
	...
	%
	By \Cref{lemma:same_coloring_no},
...
\begin{lemma}%
	...
	for state \( s = (P_\red, R_\red, P_\blue, R_\blue) \),
	\( Z = V_t \setminus \{u, v\} \),
	\( a \in \{\red, \blue\} \) and \( b \in \{\red, \blue\} \setminus \{a\} \),
	the cache value is set as follows:
	\begin{align*}
		c[t, s] & \coloneqq
		\begin{cases}
			1, & \text{if } \{u\}, \{v\} \in P_a \land \{u, v\} \in P_b                 \\
			   & \land \, \{(\{u\}, \{v\}),(\{v\}, \{u\})\} = R_a \land R_b = \emptyset \\
			   & \land \, \forall z \in Z : \{z\} \in P_a \land \{z\} \in P_b,          \\
			0, & \text{otherwise}.                                                      \\
		\end{cases} \\
	\end{align*}
	%
...
\begin{lemma}%
	...
	It comes from the same idea as the Cartesian product and
	NAC-product (\Cref{def:NACproduct}) where all the resulting colorings
	are NAC-colorings.
	The size of such product is multiple of the numbers of
...
\todo[inline]{Pozor na monochromatic components - projít celé}
