...
\begin{lemma}%
	...
	then for bags \( X_{t_1}, X_{t_2} \) and state spaces on them \( S_{t_t}, S_{t_2} \),
...
\begin{lemma}%
	...
	%
	If there is no edge in the graph \( G_t \), then \( c[t, s] = 0 \).
	%
	Otherwise, the cache function \( c \) satisfies for
	\( s=(P_\red, R_\red, P_\blue, R_\blue) \) the following:
	%
	\begin{align*}
		c[t, s]      & =
		\begin{cases}
...
\todo[inline]{Zarovnání na stranu - jak se dělá?}
...
\begin{lemma}%
	...
	%
	If there are no edges in the graph \( G_t \), then \( c[t, s] = 0 \).
	%
	Otherwise, the cache function for \( s=(P_\red, R_\red, P_\blue, R_\blue) \)
	satisfies that:
	%
	\begin{align*}
...
\begin{lemma}%
	...
	%
	If there is only a single edge \( e \) in \( G_t \) (the just introduced edge \( e \)),
	for each state \( s = (P_\red, R_\red, P_\blue, R_\blue) \in T \) and
	\( Z = V_t \setminus \{u, v\} \),
...
we prepare terms that will come useful later in \Cref{lemma:fpt_join_node}.
...
\begin{lemma}%
	\label[lemma]{lemma:fpt_join_merging}
	%
...
Now we define a notion of filtering
...
\begin{lemma}
	...
	%
	For each \( (s_1', s_2') \in S_1' \times S_2' \),
...
\todo[inline]{Jdu spinkat, ale minimálně \IntroduceVertexWithEdgesNode{} bude chtít péči}
