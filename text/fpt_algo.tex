...
In~parameterized algorithms, \( k \) often stands for~different parameters
...
In~dynamic programming algorithms, a~cache function is gradually computed
...
\begin{lemma}%
	...
	%
	It holds that \( (s_1', s_2') \in \mathcal{B} \) if~and only if
	there for each \( a \in \{\red, \blue \} \)
	exists \( p \in P_{a} \)
...
\begin{theorem}%
	...
	that for~a~given nice tree decomposition \( T \) of \( G \)
...
\todo{fix/improve, to tired to resolve}
%
\begin{observation}
