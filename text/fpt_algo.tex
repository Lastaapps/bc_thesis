...
\todo[inline]{Odmazat edge bagy, pokud nebudou třeba}
...
\todo[inline]{Define evaluation function}
...
\todo[inline]{Věty nedávají smysl, viz opravený text}
...
%
\begin{lemma}%
	\label[lemma]{lemma:same_coloring_no}
	%
...
\todo[inline]{Replace correspond with consistent.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Leaf node
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\( d(p) \) denotes deletion of  the vertex \( v \) from a part \( p \).
\( D(P) \) denotes deleting the vertex \( v \) from all the parts in partition \( P \)
...
\begin{proof}
	...
	we give appropriate states their values and \Cref{lemma:same_coloring_no} holds.
	%
\end{proof}
%
...
\begin{lemma}%
	...
	%
	If there is only a single edge \( e \) in \( G_t \) (the just introduced edge \( e \)) in \( E_t \),
	for each state \( s = (P_\red, R_\red, P_\blue, R_\blue) \in S_t \) and
	\( Z = V_t \setminus \{u, v\} \),
	...
	\begin{align*}
		...
		\begin{cases}
			1, & \text{if } \exists a \in \{\red, \blue\} :                                     \\
			   ...
			   & \land \, \forall z \in Z : \{z\} \in P_a \land \{z\} \in P_{\bar{a}},          \\
			0, & \text{otherwise}.                                                              \\
		\end{cases} \\
	\end{align*}
	%
...
%
\begin{definition}%
	\label[definition]{def:fpt_merging_mapping}
	%
...
\begin{lemma}%
	\label[lemma]{lemma:fpt_merging_mapping}
	%
	For a merging mapping \( M \), it holds that:
...
\begin{lemma}%
	...
	%
	For each pair \( (s_1', s_2') \in S_1' \times S_2' \),
	\( s_i' = (P_{i,\red}', R_{i,\red}', P_{i,\blue}', R_{i,\blue}') \),
	for each \( s = M(s_1', s_2') \in S_t \),
	\( s = (P_\red, R_\red, P_\blue, R_\blue) \),
	and for each \( a \in \{\red, \blue \} \),
	if there exists \( p \in P_{a} \)
	and \( \mathcal{P}_{i, a}' \subseteq P_{i, a}' \)
	such that \( \bigcup_{p' \in \mathcal{P}_{i, a}'} p' = p \)
	and there exists \( p_1', p_2' \in \mathcal{P}_{i, a}' : (p_1', p_2') \in R_{i, a}' \),
	then and only then \( (s_1', s_2') \in \mathcal{B} \).
\end{lemma}
%
\begin{proof}
...
\begin{lemma}%
	...
	If \( G_{t_1'} \) and \( G_{t_2'} \) have no edges,
	then and \( c[t, s] = 0 \) for each \( s \in S \).
	...
	%
	\begin{align*}
		c[t, s]        & = \sum_{(s_1', s_2') \in \mathcal{S}(s) \setminus \mathcal{B}} \cdot c[t_1', s_1'] \cdot c[t_{2}', s_{2}'] \\
		\text{where}   &                                                                                                            \\
...
\begin{proof}
	...
	based on correctness of \( M \) in \Cref{lemma:fpt_merging_mapping}.
	We also know that the \rbcol{}s are almost NAC-colorings consistent with \( s \)
	as we filter out bad states \( \mathcal{B} \) as shown in \Cref{lemma:fpt_join_bad_states}.
	%
...
and \( \blue \) in \( \delta_2' \).
But then \Cref{lemma:fpt_merging_mapping}
...
merging mapping (\Cref{def:fpt_merging_mapping})
...
\todo[inline]{This is most probably wrong!}
...
In \Cref{lemma:fpt_merging_mapping}, we show how to merge
...
\begin{proof}
	...
	these are merged along a path according to \Cref{lemma:fpt_merging_mapping}.
	%
...
\( C_t = D(C_{t'}) \)
where \( D \) is mapping from \Cref{lemma:fpt_forget_vertex_node}
