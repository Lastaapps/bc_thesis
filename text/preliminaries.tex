...
Therefore, there is no flexible realization iff.\ the graph is connected.
%
For \( d \ge 3 \), we can map all the vertices except two nonadjacent one
onto a line and rotate one of them around the line.
Therefore, flexible \( d \)-framework exists iff.\ the graph is not complete.
...
\begin{theorem}[\cite{polzacek_1927,laman_1970}]%
	...
	%
	A graph \( G \) is minimally rigid if and only if
	\( |E(G)| = 2|V(G)| - 3\) and
