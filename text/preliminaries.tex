...
%
\begin{lemma}[\cite{polzacek_1927,laman_1970}]%
	\label[lemma]{lemma:laman_characterization}
	A graph is minimally rigid if and only if
	\( |E(G)| = 2|V(G)| - 3\) and
	\( E(H) \le 2|V(H)| - 3 \) for any \( H \) subgraph of \( G \).
	%
	Therefore, for a graph \( G \) where \( |E(G)| < 2|V(G)| - 3 \) or
	where \( |E(G)| = 2|V(G)| - 3 \) and there exists a subgraph \( H \) such that \( E(H) > 2|V(H)| - 3 \),
	there exists a flexible realization of \( G \).
\end{lemma}
%
Based on \Cref{lemma:laman_characterization}, a polynomial algorithm
...
\begin{lemma}[\cite{legersky_original}]%
	...
	%
	Let \( G \) be a connected graph such that \( |E(G)| \ge 2 \). If there is \( E_c \subseteq E(G) \)
