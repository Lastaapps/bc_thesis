...
\begin{frame}
	...
	\begin{itemize}
		\item
		      ...
		      and a vertex bag \( X_t \).
		      % By \( G_t \) we denote a graph already formed by child nodes.
		\item
		      Let us have a part \( P \in \mathcal{P}(X_t) \)
		      and a binary relation \( R \) on \( P \).
		\item
		      We define a state \( s \) as a tuple \( P_\red, R_\red, P_\blue, R_\blue \).
		      We define state space \( S_t \) of \( t \) as the set of all such states \( s \).
		\item
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item
		      ...
		      on \( G_t \) if for both halves, w.l.o.g.\ for red, it holds that:
		      \begin{itemize}
			      \item
...
\begin{frame}
	...
	\begin{itemize}
		\item
		      There are not edges in \( G_t \) among vertices in \( X_t \).
		\item
		      Merge partitions using reflex transitive closure
		      while avoiding almost cycles.
		\item
		      Let \( S_1' \subset S_{t_1'}, S_2' \subset S_{t_2'} \) be states yielding \( s \) after merging:
		      %
		      \begin{align*}
			      c[t, s] & = \sum_{(s_1', s_2') \in S_1' \times S_2'} c[t_1', s_1'] \cdot c[t_{2}', s_{2}'].
		      \end{align*}
	\end{itemize}
\end{frame}
